...
def _resolve_model_file(filename: str, fallback_subdir: str = "models_parquet") -> str:
    base = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(base, "models", filename),
        os.path.join(base, "..", "models_parquet", filename),
        os.path.join(base, fallback_subdir, filename),
        os.path.join(base, "..", "WEBRTC2_Sambhav_WebRTC_Connected", "WEBRTC2", "backend_parquet_trained", "models_parquet", filename),
    ]
    for c in candidates:
        if os.path.exists(c):
            return c
...
MODEL_PATH = os.getenv("MODEL_PATH", _resolve_model_file("saanket_parquet_bilstm.keras"))
LABEL_PATH = os.getenv("LABEL_PATH", _resolve_model_file("label_mapping.json"))
MEAN_PATH = os.getenv("MEAN_PATH", _resolve_model_file("mean.npy"))
STD_PATH = os.getenv("STD_PATH", _resolve_model_file("std.npy"))
...
MAX_FRAMES      = 60
RAW_FEATURES    = 126   # landmarks only
FEATURES        = 252   # landmarks + velocity
...
if model.input_shape[2] != FEATURES:
    raise ValueError(
        ...
        "Re-train the model or check FEATURES constant."
    )
...
mean = np.asarray(mean).reshape(FEATURES)
std  = np.asarray(std).reshape(FEATURES)
...
# ============================================================
# MEDIAPIPE (Tasks API with Solutions fallback)
# ============================================================

detector = None
hands = None

TASK_PATH = _resolve_model_file("hand_landmarker.task")
if os.path.exists(TASK_PATH):
    try:
        from mediapipe.tasks import python as mp_tasks_python
        from mediapipe.tasks.python import vision as mp_tasks_vision
        base_options = mp_tasks_python.BaseOptions(model_asset_path=TASK_PATH)
        options = mp_tasks_vision.HandLandmarkerOptions(base_options=base_options, num_hands=2)
        detector = mp_tasks_vision.HandLandmarker.create_from_options(options)
        print("MediaPipe Tasks HandLandmarker initialized successfully.")
    except Exception as e:
        print("MediaPipe Tasks initialization note:", e)

if detector is None:
    try:
        mp_hands = getattr(mp, "solutions", None)
        if mp_hands and hasattr(mp_hands, "hands"):
            hands = mp_hands.hands.Hands(
                static_image_mode=False,
                max_num_hands=2,
                min_detection_confidence=0.55,
                min_tracking_confidence=0.55
            )
            print("MediaPipe Solutions Hands initialized successfully.")
    except Exception as e:
        print("MediaPipe Solutions Hands note:", e)
        ...
        try:
            ...
            if results.hand_landmarks and results.handedness:
                ...
                for hand_lms, handedness in zip(results.hand_landmarks, results.handedness):
                    hand_label = handedness[0].category_name if hasattr(handedness[0], 'category_name') else (handedness[0].display_name or "Left")
                    hand_index = 0 if hand_label == "Left" else 1
                    for idx, lm in enumerate(hand_lms):
                        landmarks[hand_index, idx, 0] = lm.x
                        landmarks[hand_index, idx, 1] = lm.y
                        landmarks[hand_index, idx, 2] = lm.z
        except Exception:
            pass
            ...
            if results.multi_hand_landmarks and results.multi_handedness:
                ...
                for hand_landmarks, handedness in zip(
                    ...
                    for landmark_index, landmark in enumerate(
                        ...
                        landmarks[hand_index, landmark_index, 0] = landmark.x
                        landmarks[hand_index, landmark_index, 1] = landmark.y
                        landmarks[hand_index, landmark_index, 2] = landmark.z
        except Exception:
            pass

    return landmarks.flatten(), detected

# ============================================================
# NORMALIZE SEQUENCE
# ============================================================

def add_velocity(seq_126):
    """seq_126: (60, 126) raw landmarks → (60, 252) with velocity appended."""
    seq = np.asarray(seq_126, dtype=np.float32)
    mask  = (seq != 0).any(axis=1, keepdims=True).astype(np.float32)  # (60,1)
    delta = np.zeros_like(seq)
    delta[1:] = (seq[1:] - seq[:-1]) * mask[1:] * mask[:-1]
    return np.concatenate([seq, delta], axis=1).astype(np.float32)   # (60,252)


def normalize_sequence(sequence_252):
    seq = np.asarray(sequence_252, dtype=np.float32)
    if seq.shape != (MAX_FRAMES, FEATURES):
        raise ValueError(f"Expected ({MAX_FRAMES}, {FEATURES}), got {seq.shape}")
    mask = (seq != 0).any(axis=1, keepdims=True)
    return np.where(mask, (seq - mean) / std, 0.0).astype(np.float32)
...
def predict_sequence(sequence):
    """sequence: (60, 126) raw landmarks OR (60, 252) already with velocity."""
    if sequence is None:
        return None

    sequence = np.asarray(sequence, dtype=np.float32)

    # Accept raw 126-feature input and add velocity automatically
    if sequence.shape == (MAX_FRAMES, RAW_FEATURES):
...
async def realtime_websocket(
    ...
    try:
        ...
        while True:
            ...
            sequence = np.asarray(frame_buffer, dtype=np.float32)  # (60, 126)
            ...
            # Convert the confirmed ISL gloss immediately so the
            # WebRTC frontend can display English without a second
            # request/round trip.
...
def extract_video_sequence(
    ...
    if len(frames) >= MAX_FRAMES:
        ...
        padding = np.zeros(
            (MAX_FRAMES - len(frames), RAW_FEATURES),
            dtype=np.float32
        )
...
def root():
    ...
    return {
        ...
        "dataset":
            "Parquet keypoint dataset only",

        "classes":
            len(label_mapping),

        "input_shape":
            "(60, 252) [landmarks + velocity]",
...
def health_status():
    ...
    return {
        "status": "healthy",
        "service": "SAANKET BiLSTM Parquet Recognition Service",
...
async def predict_landmarks_api(req: LandmarkSequenceRequest):
    ...
    try:
        ...
        if len(seq_array) != MAX_FRAMES and len(seq_array) > 1:
            indices = np.linspace(0, len(seq_array) - 1, MAX_FRAMES).astype(int)
            seq_array = seq_array[indices]
        elif len(seq_array) == 1:
            seq_array = np.repeat(seq_array, MAX_FRAMES, axis=0)

        # Run prediction
        result = predict_sequence(seq_array)
        ...
        top2 = top_preds[1]["word"] if len(top_preds) > 1 else top1
        ...
        return {
            ...
            "top2_label": top2,
            "margin": result["confidence"] - top2_conf,
